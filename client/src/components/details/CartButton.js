            ...
            .map((variation) => (
              <div key={variation?._id} className="relative">
                ...
                {selectedUnit?.unit._id === variation.unit._id ? <TiTickOutline className="w-8 h-8 text-white" /> :"" }      
                  {/* Tooltip */}
                  <div
                    className={`absolute top-full right-1/2 -translate-x-1/2 mt-3  px-2 py-1  bg-secondary text-white text-sm rounded shadow-lg transition-opacity duration-300  whitespace-nowrap
                  ${
                    selectedUnit?.unit._id === variation.unit._id
                      ? "opacity-100 visible"
                      : "opacity-0 invisible group-hover:opacity-100 group-hover:visible"
