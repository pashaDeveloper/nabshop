    ...
    features: [
      {
        icon: {
          type: String,
          maxLength: [2, "آیکون نمی‌تواند بیشتر از ۲ کاراکتر باشد"],
          default: "" // در صورت نیاز
        },
        title: {
          type: String,
          ...
          validate: [
            {
              ...
              message: "هر محتوای ویژگی نمی‌تواند بیشتر از ۲۰۰ کاراکتر باشد"
            }
          ]
        }
      }
...
productSchema.pre("save", async function (next) {
  ...
  try {
    ...
    const productUrl = `${defaultDomain}/product?product_id=${
      this._id
    }&product_title=${this.title.replace(/ /g, "-").toLowerCase()}`;
    ...
    if (
      ...
      try {
        ...
        if (category && category.title) {
          ...
          this.metaDescription =
            `${this.summary} | ${category.title}`.substring(0, 160);
        } else {
        ...
        if (!this.metaKeywords || this.metaKeywords.length === 0) {
          const tags = await Tag.find({ _id: { $in: this.tags } }).select(
            "title"
          );
          this.metaKeywords =
            tags.length > 0
              ? tags.map((tag) => tag.title).slice(0, 10)
              : ["محصول جدید"];
        }
      } catch (error) {
        console.error(
